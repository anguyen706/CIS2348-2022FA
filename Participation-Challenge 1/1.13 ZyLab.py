...
# 1.13 Interleaved input / output

# Auto-graded programming assignments have numerous advantages, but have some challenges too.
# Students commonly struggle with realizing that example input / output provided in an assignment's specification
# interleaves input and output, but the program should only output the output parts.
...
# Try submitting it for grading (click "Submit mode", then "Submit for grading"). Notice that the test cases fail.
# The first test case's highlighting indicates that output 3 and newline were not expected.
#
# In the second test case, the -5 and newline were not expected.
